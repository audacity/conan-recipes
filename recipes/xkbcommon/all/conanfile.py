...
class XkbcommonConan(ConanFile):
    ...
    def requirements(self):
        self.requires("xkeyboard-config/system@audacity/stable")
        if self.options.with_x11:
            self.requires("xorg/system@audacity/stable")
        if self.options.get_safe("xkbregistry"):
            self.requires("libxml2/2.11.4@audacity/stable")
        if self.options.get_safe("with_wayland"):
            self.requires("wayland/1.22.0@audacity/stable")
            if not self._has_build_profile:
                self.requires("wayland-protocols/1.31@audacity/stable")
    ...
    def build_requirements(self):
        ...
        self.tool_requires("bison/3.8.2@audacity/stable")
        ...
        if self._has_build_profile and self.options.get_safe("with_wayland"):
            self.tool_requires("wayland/1.22.0@audacity/stable")
            self.tool_requires("wayland-protocols/1.31@audacity/stable")
